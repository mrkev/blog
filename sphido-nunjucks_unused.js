// Based off: https://github.com/sphido/sphido/blob/master/packages/sphido-nunjucks/lib/nunjucks.js
...
const defaultTemplate =
  '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{{page.title}}</title></head><body><main><article>{{page.content|safe}}</article></main></body></html>';
...
async function save(dir, template = "theme/page.html") {
  ...
  return renderToFile(
    ...
    template.endsWith(".html") && !existsSync(template)
      ? defaultTemplate
      : template,
    { page: this }
  );
